...
class WP_SEO {
	...
	public function post_meta_fields( $post ) {
		...
		?>	
			<h4 class="is_empty"><?php esc_html_e( 'This post needs revision', 'wp-seo' ); ?></h4><span id="general_signal"></span>
			<table class="wp-seo-post-meta-fields">
				<tbody>
					<tr>
						<th scope="row"><label for="wp_seo_meta_objectivew"><?php esc_html_e( 'Palabras Objetivo', 'wp-seo' ); ?></label></th>
						<td>
							<input type="text" id="wp_seo_meta_objectivew" name="seo_meta[objective]" value="<?php echo esc_attr( $objective = get_post_meta( $post->ID, '_meta_objective', true ) ); ?>" size="96" />
							<div>
								<?php esc_html_e( 'No. Palabras objetivo: ', 'wp-seo' ); ?>
								<span class="objective-character-count"></span>
								<noscript><?php echo esc_html( $this->noscript_character_count( $objective ) ); ?></noscript>
								<span>
									<em>Debe tener máximo 4 palabras y <strong>SIN </strong>preposiciones</em>
								</span>
							</div>
						</td>
					</tr>
					...
					<tr>
						...
						<td>
							<textarea id="wp_seo_meta_keywords" name="seo_meta[keywords]" rows="2" cols="96"><?php echo esc_textarea( $keywords = get_post_meta( $post->ID, '_meta_keywords', true ) ) ?></textarea>
							<div>
								<?php esc_html_e( 'Meta Keywords word count: ', 'wp-seo' ); ?>
								<span class="keywords-character-count"></span>
								<noscript><?php echo esc_html( $this->noscript_character_count( $keywords ) ); ?></noscript>
								<span>
									<em>Debe tener entre  6 - 12 palabras</em>
								</span>
							</div>
						</td>
					</tr>
			...
			<table id="seo_indicator">
				<tbody>
					...
					<!--SEMAFORO SEO-->
					<tr id="objectivew_status">
						<td >
							<div>
								<?php esc_html_e( 'SEO Objective words Status: ', 'wp-seo' ); ?>
							</div>
						</td>
						<td>
					...
					<tr id="title_status">
						<td >
							<div>
								<?php esc_html_e( 'SEO Title Status: ', 'wp-seo' ); ?>
							</div>
						</td>
						<td>
							<div class="signal"></div>
					...
					<tr id="description_status">
						<td>
							<div>
								<?php esc_html_e( 'SEO Description Status: ', 'wp-seo' ); ?>
							</div>
						</td>
						<td>
							<div class="signal"></div>
					...
					<tr id="keyword_status">
						<td >
							<div>
								<?php esc_html_e( 'SEO Keywords Status: ', 'wp-seo' ); ?>
							</div>
						</td>
						<td>
							<div class="signal"></div>
					...
					<tr id="body_status">
						<td >
							<div>
								<?php esc_html_e( 'SEO Body Status: ', 'wp-seo' ); ?>
							</div>
						</td>
						<td>
							<div class="signal"></div>
					...
					<tr id="alt_status">
						<td >
							<div>
								<?php esc_html_e( 'SEO Alt attirbute Status: ', 'wp-seo' ); ?>
							</div>
						</td>
						<td>
							<div class="signal"></div>
					...
					<tr id="url_status">
						<td >
							<div>
								<?php esc_html_e( 'SEO URL Status: ', 'wp-seo' ); ?>
							</div>
						</td>
						<td>
							<div class="signal"></div>
					...
					<tr>
						<td class="clearance">
							<div class="ui_id">Palabras Objetivo en todos los campos</div>
						</td>
						<td class="go">
							<div class="ui_id">Óptimo</div>
						</td>
						<td class="warn">
							<div class="ui_id">Sobran palabras | Faltan coincidencias en los campos </div>
						</td>
						<td class="stop">
							<div class="ui_id">Faltan palabras | No hay coincidencias en los campos</div>
						</td>
					</tr>
				</tbody>
			</table>
	...
	public function save_post_fields( $post_id ) {
		...
		foreach ( array( 'objective', 'title', 'description', 'keywords' ) as $field ) {
			...
			if($field==='description'){
				update_post_meta($post_id, '_genesis_description', $data);
			}
		}
	}
	...
	public function edit_term_meta_fields( $tag, $taxonomy ) {
		$values = get_option( $this->get_term_option_name( $tag ), array( 'title' => '', 'description' => '', 'keywords' => '', 'objective' => '', ) );
		wp_nonce_field( plugin_basename( __FILE__ ), 'wp-seo-nonce' );
		?>
...
add_action( 'after_setup_theme', 'WP_SEO', 10, 1 );
